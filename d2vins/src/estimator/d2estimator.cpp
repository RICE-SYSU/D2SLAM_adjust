...
void D2Estimator::setStateProperties() {
    ...
    if (!params->estimate_td || state.size() < params->max_sld_win_size || 
        ...
        // ========= 核心修改开始 =========
        auto* td_param_ptr = state.getTdState(self_id);
        
        // 检查 Td 参数块是否已经存在于 Problem 中
        if (problem.HasParameterBlock(td_param_ptr)) {
            problem.SetParameterBlockConstant(td_param_ptr);
        }
        // ========= 核心修改结束 =========
    }
...
void D2Estimator::solveinDistributedMode() {
    ...
    if (params->enable_perf_output) {
        printf("[D2VINS::solveDist: beforeSolve time cost %.1f ms\n", tic.toc());
    }
    ...
    solve_count ++;
    if (params->enable_perf_output) {
        printf("[D2VINS::solveDist: total time cost %.1fms\n", tic.toc());
    }
}
